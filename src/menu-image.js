...
async function generateMenuImage({
  ...
  const ROW_H   = 90;   // product row height
  ...
  for (const { cat, items } of sections) {
    ...
    items.forEach((product, idx) => {
      ...
      ctx.textAlign = "right";
      ctx.fillText(truncate(product.name, 28), textAreaRight, midY - 10);

      // Description (if any)
      if (product.description) {
        ctx.fillStyle = COLORS.textLite;
        ctx.font = "13px Tajawal";
        ctx.fillText(truncate(product.description, 60), textAreaRight, midY + 10);
      }
      ...
      ctx.textAlign = "left";
      ctx.fillText(`${Number(product.price).toFixed(2)} ${currency}`, pad, midY + 6);

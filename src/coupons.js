 ...
 */
function validateCoupon(code, storeId, subtotal, phone) {
  const { coupons } = load();
...
function createCoupon({ code, type, value, storeId, minOrder, maxUses, expiresAt, onePerCustomer, maxDiscount }) {
  const data = load();

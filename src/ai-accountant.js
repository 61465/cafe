...
{
  "summary": "ملخص بجملتين عن الأداء العام للشهر",
  "healthScore": <رقم 0-100>,
  "advice": [
    { "priority": "high|medium|low", "type": "cost|revenue|product|operational", "title": "عنوان قصير", "detail": "توصية محددة قابلة للتنفيذ" }
  ],
  "warnings": ["تحذيرات حرجة إن وجدت"],
  "kudos": ["إيجابيات يستحق التهنئة عليها"]
}

اجعل النصائح:
- عملية ومحددة (لا عبارات عامة)
- مرتبطة بأرقام المتجر الفعلية
- مكتوبة بلغة عربية فصحى مبسطة
- بحد أقصى 5 نصائح، 2-3 تحذيرات، 1-2 إيجابيات
- استخدم العملة ر.س (ريال سعودي)`;

  try {
    const raw = await callGroq([{ role: "user", content: prompt }], { json: true, temperature: 0.3 });
